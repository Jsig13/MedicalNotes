  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-white">Note Templates</h1>
      ...
      <div className="grid grid-cols-2 gap-3">
        {templates.map((t) => (
          <Card
            key={t.id}
            className="cursor-pointer hover:border-blue-400 transition-colors stagger-item"
          >
            <div
              className="p-5"
              ...
              <div className="flex flex-wrap gap-1.5">
                {(t.sections as { id: string; title: string; order: number }[])
                  ...
                  .map((s) => (
                    <span
                      key={s.id}
                      className="text-[11px] px-2 py-0.5 rounded-md bg-slate-700/30 text-slate-400"
                    >
                      {s.title}
                    </span>
                  ))}
